...
"<button type='button' onclick='showSensorSubMenu(\"explorer\")' id='btn-modbus-explorer' style='background:#6c757d;color:white;padding:10px 15px;border:none;border-radius:4px;cursor:pointer;font-weight:bold'>🔍 Modbus Explorer</button>"
        "</div>"
        "</div>", 
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "formHtml += '<p style=\"color:#28a745;font-size:10px;margin:5px 0\"><strong>Fixed format - UINT32_3412 (CDAB) - no data type selection needed</strong></p>';"
        "} else if (sensorType === 'Piezometer') {"
        "formHtml += '<label>Scale Factor:</label><input type=\"number\" name=\"sensor_' + sensorId + '_scale_factor\" value=\"0.01\" step=\"any\" style=\"width:100px\" title=\"Piezometer scaling factor\"> <span style=\"color:#666;font-size:11px\">(0.01 for 2 implicit decimals: 14542 = 145.42)</span><br>';"
        "formHtml += '<p style=\"color:#28a745;font-size:10px;margin:5px 0\"><strong>Fixed format - UINT16_HI (16-bit) - no data type selection needed</strong></p>';"
        "}"
