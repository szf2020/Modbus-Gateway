...
#define COMPANY_TAGLINE "Industrial IoT Solutions"
#define LOGO_COLOR_1 "#1e3a8a"  // Dark blue
#define LOGO_COLOR_2 "#3b82f6"  // Light blue
...
"<style>"
":root{--p:#2c5282;--s:#4299e1;--a:#38b2ac;--ok:#38a169;--warn:#ed8936;--err:#e53e3e;--bg:#f0f8ff;--border:#e2e8f0;--txt:#2d3748}"
"*{margin:0;padding:0;box-sizing:border-box}"
"body{font-family:Rajdhani,sans-serif;background:linear-gradient(135deg,#f0f8ff,#e1f5fe);color:var(--txt);overflow-x:hidden;min-height:100vh}"
".waves{position:fixed;top:0;left:0;width:100%;height:100%;z-index:-1;overflow:hidden;pointer-events:none}"
".wave{position:absolute;width:100%;height:80px;background:linear-gradient(135deg,rgba(135,206,250,.15),rgba(173,216,230,.1));border-radius:50%}"
".wave:nth-child(1){bottom:-40px;animation:w1 15s ease-in-out infinite}"
".wave:nth-child(2){bottom:-30px;animation:w2 18s ease-in-out infinite;opacity:.7}"
"@keyframes w1{0%,100%{transform:translateX(-80px)}50%{transform:translateX(80px)}}"
"@keyframes w2{0%,100%{transform:translateX(60px)}50%{transform:translateX(-60px)}}"
"input,select,button{padding:10px 15px;margin:4px;border:2px solid var(--border);border-radius:6px;background:#fff;color:var(--txt);font-family:Rajdhani,sans-serif;transition:all .3s}"
"input:focus,select:focus{outline:0;border-color:var(--s);box-shadow:0 0 0 2px rgba(66,153,225,.1)}"
"button{background:linear-gradient(135deg,var(--p),var(--s));border:0;color:#fff;cursor:pointer;font-family:Orbitron,monospace;font-weight:600;text-transform:uppercase}"
"button:hover{transform:translateY(-1px);box-shadow:0 6px 15px rgba(44,82,130,.3)}"
"h1,h2{color:var(--p);font-family:Orbitron,monospace;font-weight:700}"
"table{border-collapse:collapse;width:100%;margin:10px 0;background:#fff;border:1px solid var(--border);border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.1)}"
"th,td{border:1px solid var(--border);padding:8px 12px;text-align:left;color:var(--txt)}th{background:#f7fafc;font-weight:bold;font-family:Orbitron,monospace;color:var(--p)}"
"tr:hover{background:rgba(66,153,225,.05)}"
".test-result{background:#fff;border:2px solid var(--a);border-radius:8px;padding:15px;margin:10px 0;box-shadow:0 2px 8px rgba(0,0,0,.1)}"
".header{display:flex;align-items:center;margin-bottom:15px;padding:20px;background:#fff;border:2px solid var(--border);border-radius:12px;box-shadow:0 4px 15px rgba(0,0,0,.1);position:relative}"
".header::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--p),var(--s),var(--a))}"
".logo{margin-right:15px;font-size:2rem;color:var(--s)}.company-info h1{margin:0;color:var(--p);font-family:Orbitron,monospace;font-size:1.8rem;font-weight:900}.company-info p{margin:5px 0;color:var(--a);font-size:.9rem;font-family:Orbitron,monospace}"
".sensor-card{background:#fff;border:2px solid var(--border);border-radius:12px;padding:20px;margin:10px 0;box-shadow:0 2px 8px rgba(0,0,0,.1);position:relative;transition:all .3s}"
".sensor-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--p),var(--a))}"
".sensor-card:hover{transform:translateY(-2px);box-shadow:0 6px 20px rgba(0,0,0,.15)}"
".sensor-card h3{margin:0 0 10px;color:var(--p);font-family:Orbitron,monospace}"
".container{display:flex;min-height:100vh}"
".sidebar{width:280px;background:linear-gradient(180deg,white,#f7fafc);color:var(--txt);padding:0;position:fixed;height:100vh;overflow-y:auto;border-right:2px solid var(--border);box-shadow:4px 0 15px rgba(0,0,0,0.1)}"
".main-content{margin-left:280px;padding:20px;flex:1}"
".menu-item{display:flex;width:100%;padding:18px 25px;color:var(--txt);text-decoration:none;border:none;background:transparent;cursor:pointer;text-align:left;font-size:15px;font-family:Orbitron,monospace;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1px solid var(--border);transition:all .3s;position:relative;overflow:hidden;align-items:center;gap:12px}"
".menu-item:hover{background:linear-gradient(135deg,rgba(66,153,225,.08),rgba(56,178,172,.08));color:var(--p);transform:translateX(8px);box-shadow:0 2px 8px rgba(0,0,0,.1)}"
".menu-item.active{background:linear-gradient(135deg,var(--p),var(--s));color:white;border-left:4px solid var(--a);box-shadow:0 4px 12px rgba(44,82,130,.3)}"
".menu-icon{margin-right:12px;font-size:1.1rem}"
".section{display:none;background:white;border:2px solid var(--border);border-radius:16px;padding:30px;margin-bottom:25px;box-shadow:0 4px 12px rgba(0,0,0,.1);position:relative;overflow:hidden}"
".section.active{display:block}.section::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--p),var(--s),var(--a))}"
".section-title{color:var(--p);font-family:Orbitron,monospace;font-size:1.6rem;font-weight:700;margin-bottom:25px;display:flex;align-items:center;gap:12px;padding-bottom:15px;border-bottom:2px solid var(--border)}"
".sidebar .header{background:linear-gradient(135deg,white,#f7fafc);border-bottom:2px solid var(--border);padding:25px 20px;margin:0;border-radius:0;box-shadow:0 2px 8px rgba(0,0,0,.1);position:relative;overflow:hidden}"
".sidebar .header::before{content:'';position:absolute;top:0;left:0;right:0;height:3px;background:linear-gradient(90deg,var(--p),var(--s),var(--a))}"
".sidebar .company-info h1{color:var(--p);font-size:1.4rem;font-family:Orbitron,monospace;font-weight:900;z-index:2;position:relative;margin-bottom:5px}"
".sidebar .company-info p{color:var(--a);font-size:.8rem;letter-spacing:1px;text-transform:uppercase;font-family:Orbitron,monospace;font-weight:500;z-index:2;position:relative}"
"#heap_usage{color:var(--warn);font-weight:bold}#wifi_status{color:var(--ok);font-weight:bold}#uptime{color:var(--s);font-weight:bold}"
".status-good{color:var(--ok)}.status-warning{color:var(--warn)}.status-error{color:var(--err)}"
...
".wifi-input-group{position:relative;display:flex;align-items:center}"
".scan-button{background:linear-gradient(135deg,var(--a),var(--s));transition:all .3s;box-shadow:0 2px 8px rgba(56,178,172,.3);border:none;color:white;font-family:Orbitron,monospace;font-weight:600}.scan-button:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(56,178,172,.4)}"
".network-item:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,0,0,.1)}"
"@media (max-width:768px){.container{display:block}.sidebar{width:100%;height:auto;position:static;overflow:visible}.main-content{margin-left:0;padding:15px}.menu-item{padding:15px 20px;font-size:14px}.section{padding:20px;margin-bottom:15px}.sensor-card{padding:15px;margin:10px 0}input,select,textarea{width:100%;box-sizing:border-box;margin:5px 0}button{padding:12px 16px;margin:5px 0;width:100%;box-sizing:border-box}table{font-size:13px;overflow-x:auto;display:block;white-space:nowrap}.company-info h1{font-size:1.2rem}.company-info p{font-size:.8rem}}"
"@media (min-width:769px){.container{display:flex}.sidebar{width:280px;position:fixed;height:100vh}.main-content{margin-left:280px;padding:20px}}"
"</style>"
"<script>"
...
"<i class='fas fa-chart-area menu-icon'></i>Overview"
"</button>"
"<button class='menu-item' onclick='showSection(\"wifi\")'>"
"<i class='fas fa-network-wired menu-icon'></i>Network Config"
"</button>"
"<button class='menu-item' onclick='showAzureSection()'>"
"<i class='fas fa-cloud menu-icon'></i>Azure IoT Hub"
"</button>"
"<button class='menu-item' onclick='showSection(\"sensors\")'>"
"<i class='fas fa-microchip menu-icon'></i>Modbus Sensors"
"</button>"
"<button class='menu-item' onclick='showSection(\"write_ops\")'>"
"<i class='fas fa-edit menu-icon'></i>Write Operations"
"</button>"
"<button class='menu-item' onclick='showSection(\"monitoring\")'>"
"<i class='fas fa-desktop menu-icon'></i>System Monitor"
"</button>"
"</div>"
...
static esp_err_t config_page_handler(httpd_req_t *req)
{
    ...
    char chunk[4096];  // Increased buffer size to accommodate larger HTML content
    
    // Overview Section with Real-time System Resources
    httpd_resp_sendstr_chunk(req, 
        "<div id='overview' class='section active'>"
        "<h2 class='section-title'><i class='fas fa-chart-area'></i>System Overview</h2>"
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "<h2 class='section-title'><i class='fas fa-network-wired'></i>Network Configuration</h2>"
        "<div class='sensor-card'>"
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "<div>"
        "<h2 class='section-title'><i class='fas fa-wifi'></i>WiFi Settings</h2>"
        "<form method='POST' action='/save_config'>"
        "<div class='sensor-card'>"
        "<div style='display:flex;gap:12px;align-items:center;margin-bottom:20px'>"
        "<button type='button' onclick='scanWiFi()' class='scan-button'>Scan Networks</button>"
        "<div id='scan-status' style='color:#666;font-size:13px;font-weight:500'></div>"
        "</div>"
        "<div id='networks' style='display:none;border:1px solid #ddd;border-radius:8px;margin-bottom:20px;background:#f8f9fa'></div>"
        "<div class='wifi-grid'>"
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "<div>"
        "<h2 class='section-title'><i class='fas fa-sim-card'></i>SIM Module Configuration (A7670C)</h2>"
        "<form onsubmit='return saveSIMConfig()'>"
        "<div class='sensor-card'>"
        "<h3>Cellular Network Settings</h3>"
        "<p style='color:#666;margin-bottom:15px'>Configure 4G cellular connectivity</p>"
        "<label>APN (Access Point Name):</label>"
        "<input type='text' id='sim_apn' name='sim_apn' value='%s' placeholder='airteliot' maxlength='63' style='width:100%%;max-width:400px;padding:8px'><br>"
        "<small style='color:#666'>Default: airteliot (Airtel) | Use jionet for Jio SIM</small><br><br>"
        "<label>APN Username (optional):</label>"
        "<input type='text' id='sim_apn_user' name='sim_apn_user' value='%s' placeholder='username' maxlength='63' style='width:100%%;max-width:400px;padding:8px'><br>"
        "<small style='color:#666'>Leave blank if not required by carrier</small><br><br>"
        "<label>APN Password (optional):</label>"
        "<input type='password' id='sim_apn_pass' name='sim_apn_pass' value='%s' maxlength='63' style='width:100%%;max-width:400px;padding:8px'><br>"
        "<small style='color:#666'>Leave blank if not required by carrier</small><br><br>"
        "</div>"
        "<div class='sensor-card'>"
    ...
    snprintf(chunk, sizeof(chunk),
        "<div id='sd_panel'>"
        "<h2 class='section-title'><i class='fas fa-sd-card'></i>SD Card Configuration</h2>"
        "<form onsubmit='return saveSDConfig()'>"
        "<div class='sensor-card'>"
        "<h3>Offline Message Caching</h3>"
        "<p style='color:#666;margin-bottom:15px'>Enable offline message caching during network outages</p>"
        "<label style='display:inline-block;cursor:pointer;padding:10px;background:#f8f9fa;border-radius:6px'>"
        "<input type='checkbox' id='sd_enabled' name='sd_enabled' value='1' %s onchange='toggleSDOptions()' style='margin-right:8px'>"
        "<span style='font-weight:600'>Enable SD Card</span>"
        "</label><br><br>"
        "<div id='sd_options' style='display:%s'>"
        "<label style='display:inline-block;cursor:pointer;padding:8px;background:#e8f4f8;border-radius:6px;margin-bottom:15px'>"
        "<input type='checkbox' id='sd_cache_on_failure' name='sd_cache_on_failure' value='1' %s style='margin-right:8px'>"
    ...
    snprintf(chunk, sizeof(chunk),
        "<div id='rtc_panel'>"
        "<h2 class='section-title'><i class='fas fa-clock'></i>Real-Time Clock (DS3231)</h2>"
        "<form onsubmit='return saveRTCConfig()'>"
        "<div class='sensor-card'>"
        "<h3>Accurate Timekeeping</h3>"
        "<p style='color:#666;margin-bottom:15px'>Maintain accurate time even during network outages</p>"
        "<label style='display:inline-block;cursor:pointer;padding:10px;background:#f8f9fa;border-radius:6px'>"
        "<input type='checkbox' id='rtc_enabled' name='rtc_enabled' value='1' %s onchange='toggleRTCOptions()' style='margin-right:8px'>"
        "<span style='font-weight:600'>Enable RTC</span>"
        "</label><br><br>"
        "<div id='rtc_options' style='display:%s'>"
        "<label style='display:inline-block;cursor:pointer;padding:8px;background:#e8f4f8;border-radius:6px;margin-bottom:10px'>"
        "<input type='checkbox' id='rtc_sync_on_boot' name='rtc_sync_on_boot' value='1' %s style='margin-right:8px'>"
        "<span>Sync System Time from RTC on Boot</span>"
        "</label><br>"
        "<label style='display:inline-block;cursor:pointer;padding:8px;background:#e8f4f8;border-radius:6px;margin-bottom:15px'>"
        "<input type='checkbox' id='rtc_update_from_ntp' name='rtc_update_from_ntp' value='1' %s style='margin-right:8px'>"
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "<small style='color:#666'>GPIO pin for configuration mode trigger (0-39, pull LOW to enter config mode)</small><br><br>"
        "<div style='background:#d4edda;padding:12px;margin:15px 0;border-radius:5px;border:1px solid #c3e6cb'>"
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "</label><br>"
        "<small style='color:#666'>When enabled, the specified GPIO will power cycle the modem (2 seconds) on MQTT disconnection</small><br><br>"
        ...
        "<small style='color:#666'>GPIO pin to control modem power (2-39, avoid 0,1,6-11 which are reserved)</small><br><br>"
        ...
        "<small style='color:#666'>Time to wait for modem to boot up after reset before reconnecting WiFi (5-60 seconds)</small><br><br>"
        "<div style='background:#d4edda;padding:12px;margin:15px 0;border-radius:5px;border:1px solid #c3e6cb'>"
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "<h2 class='section-title'><i class='fas fa-cloud'></i>Azure IoT Hub Configuration (Password Protected)</h2>"
        ...
        "<small style='color:#666'>This is configured in the firmware (read-only)</small><br><br>"
        ...
        "<small style='color:#666'>Your Azure IoT Hub device identifier</small><br><br>",
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "</div><br>"
        "<small style='color:#666'>Required: Primary key from Azure IoT Hub device registration</small><br><br>"
        "</div>",
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "<small style='color:#666'>How often to send sensor data to Azure (30-3600 seconds)</small><br><br>"
        "<div style='background:#d4edda;padding:12px;margin:15px 0;border-radius:5px;border:1px solid #c3e6cb'>"
    ...
    snprintf(chunk, sizeof(chunk), 
        ...
        "<h2 class='section-title'><i class='fas fa-microchip'></i>Modbus Sensors (Total: %d)</h2>"
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "  h += '<style>.sensor-card label { display:inline-block; width:140px; font-weight:bold; margin-bottom:5px; }</style>';"
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "  h += '<label><strong>Sensor Name *:</strong></label><input type=\"text\" name=\"sensor_' + sensorCount + '_name\" placeholder=\"e.g., Tank 1 Water Quality\" style=\"width:300px;border:2px solid #dc3545\" required><br><br>';"
        "  h += '<label><strong>Unit ID *:</strong></label><input type=\"text\" name=\"sensor_' + sensorCount + '_unit_id\" placeholder=\"e.g., TFG2235Q\" style=\"width:300px;border:2px solid #dc3545\" required><br><br>';"
        "  h += '<label><strong>Description:</strong></label><input type=\"text\" name=\"sensor_' + sensorCount + '_description\" placeholder=\"e.g., Main tank water quality monitoring\" style=\"width:300px\"><br><br>';"
        "  h += '<label><strong>Baud Rate:</strong></label><select name=\"sensor_' + sensorCount + '_baud_rate\" style=\"width:200px\">';"
        ...
        "  h += '</select><br><br>';"
        "  h += '<label><strong>Parity:</strong></label><select name=\"sensor_' + sensorCount + '_parity\" style=\"width:200px\">';"
        ...
        "  h += '<div style=\"margin-top:15px;padding:10px;background:#f8f9fa;border-radius:4px\">';"
        "  h += '<button type=\"button\" onclick=\"testNewSensorRS485(' + sensorCount + ')\" style=\"background:#17a2b8;color:white;margin:5px;padding:10px 15px;border:none;border-radius:4px;font-weight:bold\">Test RS485</button>';"
        "  h += '<button type=\"button\" onclick=\"saveSingleSensor(' + sensorCount + ')\" style=\"background:#28a745;color:white;margin:5px;padding:10px 15px;border:none;border-radius:4px;font-weight:bold\">Save Water Quality Sensor</button>';"
        "  h += '<button type=\"button\" onclick=\"removeSensorForm(' + sensorCount + ')\" style=\"background:#dc3545;color:white;margin:5px;padding:10px 15px;border:none;border-radius:4px\">Cancel</button>';"
        "  h += '<div id=\"test-result-new-' + sensorCount + '\" style=\"margin-top:10px;display:none\"></div>';"
        "  h += '</div></div>';"
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "}"
        "let formHtml = '';"
        "formHtml += '<style>.sensor-card label { display:inline-block; width:140px; font-weight:bold; margin-bottom:5px; }</style>';"
        "formHtml += '<label><strong>Name *:</strong></label><input type=\"text\" name=\"sensor_' + sensorId + '_name\" placeholder=\"e.g., anil\" style=\"width:200px;border:2px solid #dc3545\" required><br>';"
        "formHtml += '<label><strong>Unit ID *:</strong></label><input type=\"text\" name=\"sensor_' + sensorId + '_unit_id\" placeholder=\"e.g., fg1234\" style=\"width:200px;border:2px solid #dc3545\" required><br>';"
        "formHtml += '<label>Slave ID:</label><input type=\"number\" name=\"sensor_' + sensorId + '_slave_id\" value=\"1\" min=\"1\" max=\"247\" style=\"width:80px\"><br>';"
        "formHtml += '<label>Register Address:</label><input type=\"number\" name=\"sensor_' + sensorId + '_register_address\" value=\"0\" min=\"0\" max=\"65535\" style=\"width:120px\"><br>';"
        "formHtml += '<label>Quantity (registers):</label><input type=\"number\" name=\"sensor_' + sensorId + '_quantity\" value=\"1\" min=\"1\" max=\"125\" style=\"width:80px\"><br>';"
        "formHtml += '<label>Register Type:</label><select name=\"sensor_' + sensorId + '_register_type\" style=\"width:250px\">';"
        ...
        "formHtml += '</select><br>';"
        "if (sensorType !== 'ZEST') {"
        "formHtml += '<label>Data Type:</label><select name=\"sensor_' + sensorId + '_data_type\" style=\"width:280px\" onchange=\"showDataTypeInfo(this, ' + sensorId + ')\">';"
        ...
        "formHtml += '</optgroup></select><br>';"
        "formHtml += '<div id=\"datatype-info-' + sensorId + '\" style=\"background:#e3f2fd;padding:8px;margin:5px 0;border-radius:4px;font-size:11px;display:none\"></div>';"
        "} else {"
        "formHtml += '<input type=\"hidden\" name=\"sensor_' + sensorId + '_data_type\" value=\"ZEST_FIXED\">';"
        "formHtml += '<p style=\"color:#28a745;font-size:12px;margin:5px 0\"><strong>Data Format:</strong> Fixed - INT32_BE + INT32_LE_SWAP</p>';"
        "}"
        "formHtml += '<label>Baud Rate:</label><select name=\"sensor_' + sensorId + '_baud_rate\" style=\"width:120px\">';"
        "formHtml += '<option value=\"2400\">2400 bps</option><option value=\"4800\">4800 bps</option><option value=\"9600\" selected>9600 bps</option><option value=\"19200\">19200 bps</option><option value=\"38400\">38400 bps</option><option value=\"57600\">57600 bps</option><option value=\"115200\">115200 bps</option></select><br>';"
        "formHtml += '<label>Parity:</label><select name=\"sensor_' + sensorId + '_parity\" style=\"width:120px\">';"
        "formHtml += '<option value=\"none\" selected>None</option><option value=\"even\">Even</option><option value=\"odd\">Odd</option></select><br>';"
        ...
        "formHtml += '<p style=\"color:#007bff;font-size:11px;margin:5px 0\"><em>ZEST reads 4 registers: First 2 as INT32_BE, next 2 as INT32_LE_SWAP, then sums them</em></p>';"
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "editForm+='</optgroup></select><br>';"
        "}"
        "editForm+='<label>Baud Rate:</label><select id=\"edit_baud_'+sensorId+'\" style=\"width:120px\">';"
        ...
        "editForm+='</select><br>';"
        "editForm+='<label>Parity:</label><select id=\"edit_parity_'+sensorId+'\" style=\"width:120px\">';"
        ...
        "editForm+='<label>Sensor Height:</label><input type=\"number\" id=\"edit_sensor_height_'+sensorId+'\" value=\"'+sensor.sensor_height+'\" step=\"any\" style=\"width:100px;margin:5px\"> (meters or your unit)<br>';"
        "editForm+='<label>Max Water Level:</label><input type=\"number\" id=\"edit_max_water_level_'+sensorId+'\" value=\"'+sensor.max_water_level+'\" step=\"any\" style=\"width:100px;margin:5px\"> (meters or your unit)<br>';"
        "} else if (sensor.sensor_type === 'Radar Level') {"
        "editForm+='<label><strong>Max Water Level *:</strong></label><input type=\"number\" id=\"edit_max_water_level_'+sensorId+'\" value=\"'+sensor.max_water_level+'\" step=\"any\" style=\"width:100px;margin:5px;border:2px solid #dc3545\" required> (meters or your unit)<br>';"
        "} else if (sensor.sensor_type === 'ENERGY') {"
        "editForm+='<label><strong>Meter Type:</strong></label><input type=\"text\" id=\"edit_meter_type_'+sensorId+'\" value=\"'+(sensor.meter_type||'')+'\" placeholder=\"e.g., Power Meter\" style=\"width:200px;margin:5px\"> (Used as meter identifier)<br>';"
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "const dataTypeElem=document.querySelector('select[name=\"sensor_'+sensorId+'_data_type\"]');"
        ...
        "if(!slaveIdElem||!regAddrElem||!quantityElem||(sensorType !== 'ZEST' && !dataTypeElem)||!baudRateElem||!parityElem){"
        "let missingFields=[];"
        ...
        "if(sensorType !== 'ZEST' && !dataTypeElem) missingFields.push('data_type');"
        ...
        "const dataType=dataTypeElem ? dataTypeElem.value : '';"
        ...
        "resultDiv.innerHTML='<div style=\"background:#f8d7da;padding:10px;border-radius:4px;color:#721c24\"><strong>ERROR: Communication Error</strong><br>'+e.message+'<br><br>Check browser console (F12) for details.</div>';"
        "});"
        "}"
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "<h2 class='section-title'><i class='fas fa-edit'></i>Write Operations</h2>"
        "<div class='sensor-card'>"
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "<style>.write-form label { display:inline-block; width:150px; font-weight:bold; margin-bottom:8px; text-align:left; }</style>"
        "<div class='write-form'>"
        "<label>Slave ID:</label><input type='number' id='write_single_slave' min='0' max='247' value='1' style='width:60px;margin:5px'><br>"
        "<label>Register Address:</label><input type='number' id='write_single_addr' min='0' max='65535' value='0' style='width:100px;margin:5px'><br>"
        "<label>Value (decimal):</label><input type='number' id='write_single_value' min='0' max='65535' value='0' style='width:100px;margin:5px'><br>"
        "<button onclick='writeSingleRegister()' style='background:#007bff;color:white;padding:8px 16px;margin:10px 5px 5px 0;border:none;border-radius:4px'>Write Single Register</button>"
        "</div>"
        "</div>"
        "<div id='write_single_result' style='margin:10px 0;padding:10px;background:#f8f9fa;border-radius:4px;display:none'></div>"
        "</div>"
    );
    ...
    httpd_resp_sendstr_chunk(req,
        ...
        "<div style='margin:10px 0'>"
        "<div class='write-form'>"
        "<label>Slave ID:</label><input type='number' id='write_multi_slave' min='0' max='247' value='1' style='width:60px;margin:5px'><br>"
        "<label>Start Register:</label><input type='number' id='write_multi_start' min='0' max='65535' value='0' style='width:100px;margin:5px'><br>"
        "<label>Values (comma-separated):</label><br>"
        "<textarea id='write_multi_values' placeholder='Example: 1000,2000,3000' style='width:300px;height:60px;margin:5px;padding:5px;margin-left:150px'></textarea><br>"
        "<button onclick='writeMultipleRegisters()' style='background:#28a745;color:white;padding:8px 16px;margin:10px 5px 5px 0;border:none;border-radius:4px;margin-left:150px'>Write Multiple Registers</button>"
        "</div>"
        "</div>"
        "<div id='write_multi_result' style='margin:10px 0;padding:10px;background:#f8f9fa;border-radius:4px;display:none'></div>"
        "</div>"
    );
    ...
    snprintf(chunk, sizeof(chunk),
        ...
        "<h2 class='section-title'><i class='fas fa-desktop'></i>System Monitor</h2>"
        "<div class='sensor-card'>"
...
// Handler: /api/sim_test - Test SIM connection
static esp_err_t api_sim_test_handler(httpd_req_t *req) {
    httpd_resp_set_type(req, "application/json");
