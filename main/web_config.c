...
".scada-header-main{background:linear-gradient(135deg,var(--gray-700),var(--gray-900));color:white;font-weight:bold;text-align:center;padding:10px}"
".scada-header-float{background:linear-gradient(135deg,var(--primary-600),var(--primary-700));color:white;font-weight:bold;padding:8px}"
".scada-header-int{background:linear-gradient(135deg,var(--success-600),var(--success-700));color:white;font-weight:bold;padding:8px}"
".scada-header-uint{background:linear-gradient(135deg,var(--warning-600),var(--warning-700));color:white;font-weight:bold;padding:8px}"
".scada-header-float64{background:linear-gradient(135deg,#6610f2,#520dc2);color:white;font-weight:bold;padding:8px}"
".scada-header-int64{background:linear-gradient(135deg,#20c997,#17a579);color:white;font-weight:bold;padding:8px}"
".scada-header-uint64{background:linear-gradient(135deg,#dc3545,#b02a37);color:white;font-weight:bold;padding:8px}"
...
static esp_err_t save_config_handler(httpd_req_t *req)
{
    ...
    // Success header card
    httpd_resp_sendstr_chunk(req, "<div class='sensor-card' style='text-align:center;padding:var(--space-2xl);margin-bottom:var(--space-xl)'>");
    httpd_resp_sendstr_chunk(req, "<div style='font-size:64px;margin-bottom:var(--space-md)'>✅</div>");
    httpd_resp_sendstr_chunk(req, "<h1 style='color:var(--color-success);margin-bottom:var(--space-sm)'>Configuration Saved</h1>");
    httpd_resp_sendstr_chunk(req, "<p style='color:var(--color-text-secondary);font-size:var(--text-lg)'>Your settings have been saved successfully</p>");
    httpd_resp_sendstr_chunk(req, "</div>");
    ...
    // Action buttons
    httpd_resp_sendstr_chunk(req, "<div style='display:flex;gap:var(--space-md);flex-wrap:wrap'>");
    httpd_resp_sendstr_chunk(req, "<button class='btn-secondary' onclick='location.href=\"/\"' style='flex:1;min-width:200px'>[BACK] Back to Configuration</button>");
    httpd_resp_sendstr_chunk(req, "<button class='btn-success' onclick='startOperation()' style='flex:1;min-width:200px'>[START] Start Operation Mode</button>");
    httpd_resp_sendstr_chunk(req, "</div>");

    httpd_resp_sendstr_chunk(req, "</div>"); // Close main-container
